 ...
 * - nonce: ephemeral single-use pairing nonce (32 hex, 128 bits); the peer
 *   must echo it back inside the authenticated handshake to prove possession
 *   of the scanned QR (anti-replay).
 ...
 *   pinning, order §10). DISPLAY-ONLY: durable authorization keys on the
 *   full SHA-256 identity (64 hex), never this shortcode (v0.10 Phase 14).
 *
...
data class QrPairingPayload(
    ...
    companion object {
        ...
        private val LINE = Regex("^elysium-pairing\\|v(\\d{1,2})\\|([^|]{1,160})\\|([0-9a-f]{32})\\|([0-9a-f]{8})$")
