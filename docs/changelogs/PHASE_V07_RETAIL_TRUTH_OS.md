...
2. **§XVI / Phase 25 — AndroidTvAdbAdapter honesto** — `pair()` ahora es REAL: `Success` solo tras un round-trip autorizado (connect + `getprop`), con credential alias derivado del fingerprint de la clave RSA; `queryCapabilities()` ya no miente (`readable=false` porque `readState()` es null); `InputSource` eliminado de capacidades (no tiene keycode). Clase documentada como DEVELOPER_ONLY con contrato de honestidad. **Sweep residual**: los 3 stubs CONCEPT de `TvAdapters.kt` (Samsung Tizen, Sony Bravia, AndroidGoogleTv) también anunciaban `readable=true` con `readState()=null` → ahora `readable=false, subscribable=false`. `LgWebOsTvAdapter` conserva `readable=true` porque su `readState()` es una implementación real (HTTP WebOS). Grep de verificación: 0 restos de `CARRIER_FALLBACK_TOLERANCE_HZ`, `Success("adb-key")`.
...
1. **Phase 32 — MacCrypto direccional completo** — sin cambiar el wire legacy (el agente Swift actual sigue usando `elysium-channel`; cero cambios silenciosos):
   ...
   - **Twin Swift (mac-agent)**: `Crypto.swift` ahora expone `LinkSide` (phone/mac), `ChannelKeys.deriveKeys(myPrivate:theirPublic:side:)` (HKDF tx/rx idéntico), `sealToPeer/openFromPeer(ad)` con `ReplayGuard` sliding window y `channelAd(direction:version:)` — byte-por-byte idéntico al lado Kotlin, sin tocar el legacy `ChannelCipher.seal/open` ni `ConnectionHandler` (adopción coordinada Android+Mac pendiente; `NonceCounter(domain:)` por defecto 0 conserva el layout legacy). Diferencia cerrada: AAD nulo ⇒ overload sin `authenticatedData` (CryptoKit con `Data()` vacío cambiaría el tag vs. Kotlin). PENDIENTE de compilación Swift: regla de Jor (batch).
